"""Tests for the shared helpers on ``OrchidClient``.
...
from orchid_ai import OrchidClient, PendingApproval
...
def client():
    ...
    with patch("orchid_ai.client.build_graph") as build:
        ...
        yield OrchidClient(config=config, runtime=runtime)
...
class TestPrepareInvocation:
    ...
    async def test_generates_chat_id_when_missing_and_no_auto_create(self, client):
        ...
        assert isinstance(prepared.auth_ctx, AuthContext)
    ...
    async def test_uses_explicit_auth_when_provided(self, client):
        my_auth = AuthContext(access_token="T", user_id="real", tenant_key="org")
        prepared = await client._prepare_invocation(
            message="hi",
            chat_id="c",
            user_id="ignored",
...
class TestInterruptToResult:
    def test_dict_interrupt_extracts_fields(self):
        ...
        result = OrchidClient._interrupt_to_result(exc, chat_id="c")
