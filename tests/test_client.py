"""Tests for ``orchid_ai.client.OrchidClient``."""
...
from orchid_ai import InvokeResult, OrchidClient, PendingApproval
...
# ── InvokeResult / PendingApproval dataclasses ─────────────────


class TestDataclasses:
    def test_invoke_result_defaults(self):
        r = InvokeResult(response="ok", chat_id="c-1")
    ...
    def test_pending_approval_fields(self):
        p = PendingApproval(tool="t", args={"a": 1}, agent="x", interrupt_id="id-1")
        assert p.tool == "t"
        assert p.args == {"a": 1}
        assert p.agent == "x"
...
class TestInvoke:
    ...
    async def test_generates_chat_id_when_missing(self, minimal_config, noop_runtime):
        with patch("orchid_ai.client.build_graph") as build:
            build.return_value = _fake_graph(response="hello", agents=["helper"])
            client = OrchidClient(config=minimal_config, runtime=noop_runtime)
    ...
    async def test_explicit_auth_wins(self, minimal_config, noop_runtime):
        with patch("orchid_ai.client.build_graph") as build:
            ...
            custom = AuthContext(access_token="X", user_id="u-orig", tenant_key="org")
            await client.invoke(
                "hi",
                chat_id="c-1",
                user_id="ignored",
...
class TestResume:
    ...
    async def test_resume_sends_command(self, minimal_config):
        ...
        with patch("orchid_ai.client.build_graph") as build:
            ...
            build.return_value = graph
            client = OrchidClient(config=minimal_config, runtime=runtime)
...
class TestPersistence:
    ...
    async def test_persists_messages_when_enabled(self, minimal_config, noop_runtime):
        chat_repo = SQLiteChatStorage(dsn=":memory:")
...
class TestLifecycle:
    ...
    async def test_async_context_manager(self, minimal_config, noop_runtime):
        with patch("orchid_ai.client.build_graph") as build:
            ...
            async with OrchidClient(config=minimal_config, runtime=noop_runtime) as client:
