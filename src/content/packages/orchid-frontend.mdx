...
Add an entry to `agents.yaml`, redeploy `orchid-api`. The UI picks it up automatically on the next page load — no frontend code changes, no rebuild of the frontend.
...
<Sources files={["orchid-frontend/README.md", "orchid-frontend/AGENTS.md", "orchid-frontend/src/app/actions/chats.ts", "orchid-frontend/src/lib/auth/auth.ts", "orchid-frontend/src/lib/auth/oauth-provider.ts"]} />
