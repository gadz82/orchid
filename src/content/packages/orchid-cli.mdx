...
The CLI defaults to **ChromaDB** (`vector_backend: chroma`) for zero-infrastructure RAG — vectors live on disk at `~/.orchid/chroma/`, no Qdrant container required. Set `VECTOR_BACKEND=qdrant` to switch to Qdrant when you need sparse/hybrid search or scope promotion.
