...
external_agents:
  ask_assistant:
    ...
    normalizer_instruction: ""      # system prompt when normalizer: llm
```

Agents reference external-agent tools by name in their `tools:` list:
...
- **Approval gate** — `requires_approval: true` (default) triggers the existing
  HITL `interrupt()`. The operator must confirm every delegation in the terminal
  before the subprocess runs.
- **No shell** — the command runs via `asyncio.create_subprocess_exec` with an
  argv list. Never `shell=True`. The executable, flags, working directory, and
  timeout come from trusted operator YAML — only `prompt` is LLM-controlled.
- **Timeout kill** — runaway processes are killed after the configured timeout.
  The tool result is `{"error": "timeout", "timeout_s": …}`.
- **Per-tool allowlisting** — each external CLI must be explicitly declared in
  YAML. An agent only sees tools listed in its `tools:` array.
